...
- `access` — `admin` для пользователей с любым доступом к режиму и суперадминов, `superadmin` только для `ADMIN_IDS`. По умолчанию `admin`.
- `description` — текст для `/help`. По умолчанию пустая строка.

Неизвестные ключи команд считаются ошибкой конфигурации, чтобы опечатки не проходили молча. Если команда отсутствует в файле, для неё используется дефолт. `/grant` и `/revoke` не могут иметь доступ ниже `superadmin`.
