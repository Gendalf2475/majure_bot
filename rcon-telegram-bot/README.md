...
- `/chatid` доступен только суперадминам, чтобы не раскрывать ID чатов обычным пользователям.
...
Также можно ответить на сообщение пользователя. В этом формате user_id берётся строго из сообщения, на которое отвечает суперадмин:
...
Пользователь без прав получает отказ на служебные команды. Пользователь с доступом к одному режиму видит в `/help`, `/servers`, `/status` и `/players` только доступные ему режимы. `/ping`, `/chatid`, `/grant`, `/revoke` и просмотр чужих доступов через `/access <user_id>` доступны только суперадминам.
